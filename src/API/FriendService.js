...
export default class FriendService {
  static async getAllFriendLists(token) {
    return await axios.get("http://127.0.0.1:5000/friends", {
      headers: {
  ...
  static async rejectFriendRequest(token, friend_id) {
    return await axios.delete("http://127.0.0.1:5000/incoming_friend_request", {
      data: {
        friend_id: friend_id,
      },
      headers: {
  ...
  static async removeFriend(token, friend_id) {
    return await axios.delete("http://127.0.0.1:5000/friends", {
      data: {
        friend_id: friend_id,
      },
      headers: {
